const server_url = 'https://test-api-eu.onrender.com/api/v1/products'
...
function retrieveCart() {
  ...
  for (let i = 0; i < numberOfItems; i++) {
    ...
    fetch(`${server_url}/${itemObject.id}`) // Changed the API link
...
function makeArticle(item) {
  ...
  article.dataset.color =
    item.color /** Make Article by searching data by color*/
  return article
}
...
function makeCartContent(item) {
  ...
  const description =
    makeDescription(
      item
    ) /** Make description(cart__item__content__description)*/
  const settings =
    makeSettings(item) /** Make settings(cart__item__content__settings)*/
...
function addQuantityToSettings(settings, item) {
  ...
  input.addEventListener('input', (e) =>
    updatePriceAndQuantity(input.value, item, e)
  ) /** to catch the quantity in input. */
...
function displayTotalQuantity() {
  ...
  const total = cart.reduce(
    ...
    0
  ) /** to calculate total and item in cart. */
...
function displayTotalPrice() {
  ...
  const total = cart.reduce(
    ...
    0
  ) /** to calculate total and item. */
  totalPrice.textContent = total
}

/** Update product's quantity and price. */
function updatePriceAndQuantity(newValue, item, e) {
  /** newValue = updated "input.value" */
  if (newValue < 0 || newValue == null || newValue == 0 || newValue > 100) {
    /** to check if the cart is in correct number. */
  ...
  ) /** to find the id of the item updated. */
...
function addDeleteButton(settings, item) {
  ...
  div.addEventListener('click', () =>
    deleteItem(item)
  ) /** to catch the item to delete and pass to deleteItem.*/
...
function deleteItem(item) {
  const itemToDelete = cart.findIndex(
    /** to find only index of array to delete. */
    (product) => product.id === item.id && product.color === item.color
  ) /** to find the item of the id and of the color of the one to delete. */
...
function submitForm(e) {
  ...
  if (
    ...
    return /** to check if the form is valid. */
...
function makeRequestData() {
  const form =
    document.querySelector(
      '.cart__order__form'
    ) /** pick up data from the form filled in cart__order__form. */
  const contact = {
    /** to create an object with the data of the form. */
  ...
  const products = cart.map(
    (item) => item.id
  ) /** to create an array with the id of the items in cart. */
  const requestData = {
    contact,
    products,
  }
